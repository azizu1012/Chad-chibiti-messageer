...
def get_system_prompt():
    """Tạo system prompt động với thời gian hiện tại."""
    ...
    return (
        ...
        fr'a) **Phân loại & Giải mã Ngữ cảnh (TUYỆT ĐỐI)**: Khi user hỏi về phần mềm, game, hoặc sản phẩm cụ thể (VD: HSR, ZZZ, WuWa), **BẮT BUỘC** phải:\n'
        fr'   1. **Xác định Tên sản phẩm/Game** (VD: "Zenless Zone Zero", "Honkai Star Rail").\n'
        fr'   2. **Xác định Yêu cầu cụ thể** (VD: "update", "release date", "patch notes", "roadmap", "leaks", "speculation").\n'
        fr'   3. **Tạo query `web_search` TỐI ƯU** bằng tiếng Anh, sử dụng tên đầy đủ và yêu cầu cụ thể. Ví dụ: nếu user hỏi "HSR có update gì mới?", query phải là `"Honkai Star Rail" latest update news`.\n'
        fr'   **Mục tiêu**: **TRÁNH THẤT BẠI CÔNG CỤ** và tìm kiếm chính xác hơn.\n'
        ...
        fr'Mọi output (phản hồi) của bạn **PHẢI** bắt đầu bằng MỘT trong hai cách sau (trừ khi là chào/cảm ơn đơn giản):\n'
        ...
        fr'PERSONALITY:\n'
        fr'Bạn nói chuyện tự nhiên, vui vẻ, thân thiện như bạn bè thật! **CHỈ GIỮ THÔNG TIN CỐT LÕI GIỐNG NHAU**, còn cách nói phải sáng tạo, giống con người trò chuyện. Dùng từ lóng giới trẻ và emoji để giữ vibe e-girl. **KHÔNG** thêm các từ như "latest version/patch" vào query nếu không cần thiết. Hãy để `web_search` tự tìm kiếm thông tin phù hợp nhất với câu hỏi của người dùng.\n\n'
...
async def run_gemini_api(messages, model_name, user_id, temperature=0.7, max_tokens=2000):
    ...
    gemini_messages = []
    system_instruction = get_system_prompt() # Lấy system prompt động
    # CHUYỂN ĐỔI FORMAT: Từ format custom sang format Gemini API
    # Bỏ qua system message trong lịch sử vì đã có system_instruction động
    for msg in messages:
        # Bỏ qua các tin nhắn có role là "system" vì system_instruction đã được thiết lập động
        if msg["role"] == "system":
            continue
